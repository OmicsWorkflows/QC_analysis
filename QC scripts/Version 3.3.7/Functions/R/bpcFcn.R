...
getBPCmzML <- function(x, suffix.csv = '.csv', 
  ...
  if(length(flist$calc) > 0) {
    res <- lapply(flist$calc, function(y) calculateChromatogramMzML(y, ms.level = 1, dir = config$file.input,
...
getBPCraw <- function(x, suffix.csv = '.csv', 
  ...
  if(length(flist$calc) > 0) {
    res <- lapply(flist$calc, function(y) calculateChromatogramRaw(y, ms.level = 'ms',
