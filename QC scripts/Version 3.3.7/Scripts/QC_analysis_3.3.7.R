...
#----------------- CALCULATE TICS ---------------
        ...
        names(errors)[length(errors)] <- paste(msn, 'level')

        if(opts$qc.bool){
          qc.not.calc <- setdiff(samples, unique(out$qc.df$sample))
...
#------------------ QC METRICS ------------------
        ...
        # Correlation matrix
          ...
          img_tbl[nrow(img_tbl)+1,] <- plotCorrelationMatrix(out$tic.df, 'RT', 'intensity', scale.center = opts$plot.corr.center)
        }
...
#------------------ TSV OUTPUT ------------------
      ...
      opts$file.report <- replaceTmpFolder()
...
if(opts$open.report) { del <- system(paste0('"', browser_path, '" "', opts$file.report, '"')) }
