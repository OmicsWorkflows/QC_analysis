...
#------------------ RAW FILE LOOKUP ----------------
    ...
    fn <- fn[endsWith(fn, opts$file.format)]
...
#-------------- PLOT TICS OVERLAY ---------------
      ...
      overlay_chunks <- split(unique(df$sample), 
                              ceiling(seq_along(unique(df$sample))/opts$plot.max.samples))
      img_tbl <- rbind(img_tbl, plotTicOverlay(overlay_chunks, df, 'RT', 'intensity', start = opts$calibration.start,
...
#------------------ QC METRICS ------------------
      ...
      img_tbl[nrow(img_tbl)+1,] <- plotFluctuations(data_fluc$data, limits = 10, opts)
      
#--------------- INJECTION AMOUNT ---------------
      if(tolower(msn) == 'ms' & opts$calibration.bool) {
        export_tbls$inject_tbl_exp <- export_tbls$sums_tbl |>
...
#---------------- PROCESS AUC -------------------
    # Rename colnames in sums and HTMLize
    export_tbls$sums_tbl <- export_tbls$sums_tbl |>
      spread(key = Level, value = Value)
    export_tbls$sums_tbl_exp <- as.character(tableHTML(export_tbls$sums_tbl, 
                                                       rownames = FALSE, border = 0))
    if(opts$calibration.bool) export_tbls$sums_tbl_exp <- paste0(export_tbls$sums_tbl_exp, '<br>', export_tbls$inject_tbl_exp)
    
#----------------- HTML OUTPUT ------------------
    # Create HTML ouput
    header <- paste('<b>Sample batch:</b>', b, '\n<br>')
    createHTMLreport(header, title = 'QC sample analysis', img.table = img_tbl,
