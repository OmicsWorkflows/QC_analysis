...
getCalibrationInfo <- function(names, d, opts) {
  ...
  if(opts$calibration.tag.read == 'default') {
    ...
    if(is.null(default.tag)) {
      ...
      config$calibration.start <- calibration.tbl.info$`TIC curve start`
      config$calibration.end <- calibration.tbl.info$`TIC curve end`
...
calculateInjection <- function(d, equation, max) {
  ...
  if(any(d$Note == '*')) {
    d_txt <- gsub('<td id="tableHTML_column_4">*</td>', '<td bgcolor="#f59105" id="tableHTML_column_4">*</td>', d_txt, fixed = TRUE)
    d_txt <- paste0(d_txt,
  ...
  if(any(d$Note == '&dagger;')) {
    d_txt <- gsub('<td id="tableHTML_column_4">&dagger;</td>', '<td bgcolor="#eb3f1c" id="tableHTML_column_4">&dagger;</td>', d_txt, fixed = TRUE)
    d_txt <- paste0(d_txt,
