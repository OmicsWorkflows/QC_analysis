...
def getQCpeptide(f, all_frames, m_list):
...
def readQCpeptides(file, matrix, frames, output):
    ...
    list_qc = getQCpeptide(file, frames, mz_list)
...
def readRawBrukerTimsMs2(input, file, msn, qc_path, output, qc_output, qc_only):
    writeSysInfo(sys_info_output, 'start')
    ...
    spec = timsrust_pyo3.read_all_spectra(path)
    writeSysInfo(sys_info_output, 'spectra read')
