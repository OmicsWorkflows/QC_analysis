...
adjustDfCalibration <- function(d, config = opts) {
  d$sample <- sapply(d$sample, function(x) config$amounts[which(unique(d$sample) == x)], 
                     USE.NAMES = FALSE)
...
getCalibrationCurveInfo <- function(opts) {
  ...
  if(length(where) == 0) {
    say(paste('Could not find a calibration curve plot for calibration curve', opts$calibration.tag),
        type = 'warn')
    return(NULL)
  } else {
