...
createFolders <- function(opts, ms.levels) {
  ...
  if(opts$file.format == '.csv') {
    output$file.input <- addSlash(paste0(t, ms.levels[1]))
  }
...
createExportTables <- function(d, ms.dict, removed = NULL){
 ...
 # Creates first table with sample information
  ...
  return(list(samples_tbl = samp,
              samples_tbl_exp = samp_txt,
              sums_tbl = sums))
}
...
createTmpDirectory <- function(batch.name = opts$file.batch, output = opts$file.output,
  ...
  dir_name <- removeSpecialCharacters(dir_name)
  ...
  if(dir.exists(path)) {
    files_in_path <- list.files(path, recursive = TRUE, full.names = TRUE)
    file.remove(files_in_path)
...
replaceTmpFolder <- function(tmp.folder = opts$sample.dir, batch.name = opts$file.batch, 
  ...
  if(dir.exists(full_name)){
    ...
    say(paste0('Could not create folder ', full_name, ', all files kept in temp folder'))
    return(tmp.folder)
  }
}
