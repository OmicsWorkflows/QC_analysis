...
#----------------- CONFIGURATION ----------------
  ...
  fn <- sapply(opts$amounts, function(x) {
    list.files(opts$file.input,
               full.names = TRUE, recursive = TRUE)[grep(paste0('QC_cal', x, 'ng-', opts$method, '-', opts$date),
                                                         list.files(opts$file.input, full.names = TRUE, recursive = TRUE), 
                                                         fixed = TRUE)]},
    USE.NAMES = TRUE)
  ...
  if(length(samples) == 0) {
    ...
    say(paste('No samples with filename pattern',
              paste0('QC_cal*ng-', opts$method, '-', opts$date), 'found'),
        type = 'error')
    quit()
  }
