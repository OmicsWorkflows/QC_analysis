...
calculateQCmzML <- function(x, dir, output.dir, matrix, 
  ...
  tryCatch({
    ...
    tmp <- readMS |> filterFile(paste0(x, '.mzML'))
    ...
    say('Warning!')
    say(paste('Could not process QC data for sample', x), type = 'warn')
    return(NULL)
  })
}
...
getQCs <- function(x, opts, suffix.csv = '.csv') {
  ...
  if(length(flist$calc) > 0) {
    if(opts$file.format == '.mzML') {
      res <- lapply(flist$calc, function(y) calculateQCmzML(y, dir = opts$file.input,
                              identify.bool = opts$qc.identify,
                              output.dir = opts$qc.dir,
                              matrix = opts$qc.reference.matrix))
    } else if(opts$file.format == '.raw'){
      res <- lapply(flist$calc, function(y) {
        calculateQCraw(y, dir = opts$file.input,
                       matrix = opts$qc.reference.matrix, 
                       identify.bool = opts$qc.identify,
                       output.dir = opts$qc.dir)
      })
