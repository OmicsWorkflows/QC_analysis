...
calculateChromatogramMzML <- function(x, ms.level = 1, dir, output.dir, aggregation,
  ...
  tryCatch({
    ...
    # save as data frame
    date <- getDateFromMzML(paste0(dir, x, '.mzML'), config$file.date)
    ...
    say('Warning!')
    say(paste('Could not process', type, 'data for sample', x), type = 'warn')
    return(NULL)
  })
}
...
getTICmzML <- function(x, type, ms.level = msn, suffix.csv = '.csv',
  ...
  if(length(flist$load) > 0) {
    ...
    say(paste('Previously calculated TIC data loaded from', subdir))
  }
  
  if(length(flist$calc) > 0) {
    res <- lapply(flist$calc, function(y) calculateChromatogramMzML(y, dir = config$file.input, 
                                      output.dir = subdir, ms.level = ms.val, aggregation = 'sum', config))
...
calculateChromatogramRaw <- function(x, ms.level = msn, sample.name = 'sample', 
  ...
  tryCatch({
    ...
    say('Warning!')
    say(paste('Could not process', cr.type, 'data for sample', x), type = 'warn')
    return(NULL)
  })
}
...
getTICraw <- function(x, type, ms.level = msn, suffix.csv = '.csv', 
  ...
  if (length(flist$calc) > 0) {
    res <- lapply(flist$calc, function(y) as.data.frame(calculateChromatogramRaw(y, output.dir = subdir, cr.type = 'tic', config = config)))
