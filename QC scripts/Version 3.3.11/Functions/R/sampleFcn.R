...
getSampleData <- function(x, opts) {
  ...
  for (msn in names(opts$MS.levels)) {
  ...
  if (length(data) > 0) {
    ...
    other.dirs <- other.dirs[!other.dirs %in% c('QC_data', 'tic_data', 'bpc_data')]
        ...
        if(length(subdirs) > 0) {
          for(subdir in subdirs) {
            ...
            if(!is.null(tmp)) {
              ...
              if(nrow(colors) > 1) {
                color <- colors$colors[colors$subtype == subtype]
                if(length(color) == 0) color <- 'dimgray'
                tmp$color <- color
              } else {
                color <- colors$colors
                if(length(color) == 0) color <- 'dimgray'
          ...
          if(!is.null(tmp)) {
            color <- lcColors$colors[lcColors$type == type]
            if(length(color) == 0) color <- 'dimgray'
            tmp$color <- color
            tmp$type <- type
          }
    ...
    if (opts$qc.bool) {
      ...
      # Create a dataframe with colors for individual QC peptides
      qcColors.df <- data.frame(QC = unique(data$`QC peptides`$QC))
      all.colors <- rep(qcColors, times = ceiling(nrow(qcColors.df)/length(qcColors)))
      qcColors.df$color <- all.colors[seq_along(qcColors.df$QC)]
      data$`QC peptides` <- merge(data$`QC peptides`, qcColors.df)
    }
...
plotSampleData <- function(s, l, opts) {
  ...
  tic.names <- c(paste('TIC', names(opts$MS.levels)), 'BPC')
  if(length(tic.names) > 2) tic.names <- tic.names[c(1, length(tic.names), 2:(length(tic.names)-1))]
  ...
  additional.traces <- names(l)[!names(l) %in% c('QC peptides', 'BPC',
                                                 paste('TIC', names(opts$MS.levels)),
                                                 'TIC fluctuation')]
  if(length(additional.traces) > 0) {
    ...
    for (trace in additional.traces){
      ...
      colnames(tmp)[2] <- tools::toTitleCase(colnames(tmp)[2])
  ...
  if(opts$qc.bool) {
    tmp.p <- createSeparateQCplot(s, l$`QC peptides`, 'QC peptides', which(samples == s), opts$qc.identify,
