...
plotFluctuations <- function(d, limits, tic.opts = opts, subtitle = opts$plot.subtitle,
                             ms.level = names(msn),
                             filename = paste0(removeSpecialCharacters(opts$file.batch), '_', names(msn), '_fluctuation.png')) {
...
plotAUC <- function(d, x, y, 
                    subtitle = opts$plot.subtitle, ms.level = names(msn), 
                    filename = paste0(removeSpecialCharacters(opts$file.batch), '_', names(msn), '_AUC.png'),
                    tic.opts = opts) {
  
  colnames(d)[which(colnames(d) == x)] <- 'x'
  colnames(d)[which(colnames(d) == y)] <- 'y'
  
  d$x <- sapply(d$x, function(z) tic.opts$sample.table$sample.adj[tic.opts$sample.table$sample == z])
...
# Plot corr matrix
plotCorrelationMatrix <- function(d, x, y, rt.decimals = 1, scale.center = 'Auto',
                                  subtitle = opts$plot.subtitle, ms.level = names(msn)) {

  colnames(d)[which(colnames(d) == x)] <- 'x'
  colnames(d)[which(colnames(d) == y)] <- 'y'
  
  d$sample <- sapply(d$sample, function(x) opts$sample.table$sample.adj[opts$sample.table$sample == x])
  ...
  m <- cor(d, use = "na.or.complete") |> as.data.frame()
  
  # Cluster samples
  if(ncol(m) > 2) {
    cl <- hclust(dist(m))
    sample.order <- cl$labels[rev(cl$order)]
  } else {
    sample.order <- colnames(m)
  }
  ...
  # Get scale center
  if(scale.center == 'Auto') {
    center.type <- 'automatic'
    center.value <- min(m$corr.coef)
  } else {
    center.type <- 'manual'
    center.value <- (2*as.numeric(scale.center))-1
  }

  m$x <- factor(m$x, levels = sample.order)
  m$y <- factor(m$y, levels = sample.order)
  
  p <- ggplot(m, aes(x = x, y = y, fill = corr.coef, label = round(corr.coef, digits = 2))) +
    geom_tile() +
    labs(title = paste0('Sample correlation matrix (', center.type, ' scale)'), subtitle = subtitle) +
    ...
    scale_fill_gradientn(colours = c('#b2182b', '#ef8a62', '#fddbc7', '#f7f7f7', 
                         ...
                         limit = c(center.value, 1), space = "Lab", 
                         name="Pearson\ncorrelation") +
    coord_fixed() +
  ...
  # Note this plot in image table
  tab <- tibble(name = paste0('Sample correlation matrix (', center.type, ' scale)'),
                link = paste0('corr_matrix_', center.type),
                plot = list(p),
                filename = filename, 
                width = width, 
                height = height,
...
# AUC quantiles
plotAUCquantiles <- function(df, quantile.no, opts) {
  # calculate RT boundaries
  rt.range <- c(min(df$RT), max(df$RT))
  rt.diff <- (rt.range[2]-rt.range[1])/quantile.no
  
  rt.df <- data.frame(start = rt.range[1] + 0:(quantile.no-1)*rt.diff,
                      end = rt.range[2] - (quantile.no-1):0*rt.diff)
  
  df$sample <- sapply(df$sample, function(x) opts$sample.table$sample.adj[opts$sample.table$sample == x])
  
  # calculate AUC for separate RT ranges
  res <- sapply(unique(df$sample), function(x) {
    tmp <- subset(df, sample == x)
    apply(rt.df, 1, function(y) {
      start <- unname(y[1])
      end <- unname(y[2])
      subtmp <- subset(tmp, RT >= start & RT <= end)
      ticAUC(subtmp$RT, subtmp$intensity)
    })
  }, USE.NAMES = TRUE)
  
  colnames(res) <- unique(df$sample)
  rownames(res) <- paste0('Q', 1:quantile.no)
  
  final <- res |> as.data.frame() |> rownames_to_column('quantile') |>
    gather(-quantile, key = 'sample', value = 'AUC') |>
    group_by(sample) |>
    mutate(percentage = (AUC/sum(AUC, na.rm = FALSE))*100)
  
  # plot bar chart
  ggplot(final, aes(x = sample, y = AUC, fill = quantile)) +
    geom_col(position = position_dodge2()) +
    labs(title = 'AUC in RT quantiles',
         subtitle = opts$plot.subtitle,
         fill = 'Quantiles') +
    scale_y_continuous(expand = expansion(mult = c(0,0.15))) +
    theme_bw() +
  ...
  ggplot(final, aes(x = sample, y = percentage, fill = quantile)) +
  ...
  # calculate ratios
  quantiles <- unique(final$quantile)
