...
readConfig <- function(x) {
  ...
  opts$file.wait.time <- ifelse(cfg[cfg$variable == 'wait time between loops (min)', 2] %in% c('Inf', 'NA', 'N', 'inf', 'na', 'Na', '0'), 
                                NA, as.numeric(cfg[cfg$variable == 'wait time between loops (min)', 2]))
  if('max number of empty loops' %in% cfg$variable) {
    opts$max.loops <- ifelse(cfg[cfg$variable == 'max number of empty loops', 2] %in% c('Inf', 'NA', 'N', 'inf', 'na', 'Na', '0'), 
                             NA, as.numeric(cfg[cfg$variable == 'max number of empty loops', 2]))
  } else {
    opts$max.loops <- NA
  }
  
  if('open .html report automatically (Y/N)' %in% cfg$variable) {
    opts$open.report <- ifelse(cfg[cfg$variable == 'open .html report automatically (Y/N)', 2] == 'Y', TRUE, FALSE)
  } else {
    opts$open.report <- FALSE
  }
  if('file limit' %in% cfg$variable){
    opts$file.limit <- as.numeric(cfg[cfg$variable == 'file limit', 2])
  } else {
    opts$file.limit <- 0
  }
  ...
  if(opts$file.format == '.raw') {
    lvls_l <- c('ms', 'ms2')
    names(lvls_l) <- c('MS', 'MS2')
  } else {
    lvls_l <- c(1, 2)
    names(lvls_l) <- c('MS', 'MS2')
  }
  
  lvls_l <- lvls_l[lvls_str]
  opts$MS.levels <- lvls_l
  ...
  if('m/z tolerance' %in% cfg$variable) {
    opts$qc.mz.tolerance <- as.numeric(cfg[cfg$variable == 'm/z tolerance', 2])
  } else {
    opts$qc.mz.tolerance <- 0.015
  }

  # configuration of plots
  if('plot width (px)' %in% cfg$variable) {
    opts$single.plot.width <- as.numeric(cfg[cfg$variable == 'plot width (px)', 2])
  } else {
    opts$single.plot.width <- 500
  }
  ...
  opts$plot.corr.center <- as.numeric(cfg[cfg$variable == 'correlation matrix center', 2])
  
  if('analysis quantiles' %in% cfg$variable) {
    opts$plot.quantiles <- as.numeric(cfg[cfg$variable == 'correlation matrix center', 2])
  } else {
    opts$plot.quantiles <- 4
  }
...
getSampleNames <- function(batch, config = opts) {
  if(config$file.format != '.d') {
    all.files <- list.files(config$file.input, recursive = TRUE, full.names = TRUE)
  } else {
    all.files <- list.dirs(config$file.input, recursive = TRUE, full.names = TRUE)
  }
  ...
  fn.short <- sapply(fn, function(x) {
    x.split <- unlist(strsplit(x, '/', fixed = TRUE))
  ...
  fn <- fn[grepl(lookup, fn.short)]
  
  # Get file times and filter based on selected time offset
  if(config$file.format == '.d') { 
  ...
  fn <- fn[order(fn.times)]
  fn.times <- sort(fn.times)
  ...
  # Only use X number of files, if selected
  if(config$file.limit > 0 & length(fn) > config$file.limit) {
    fn <- fn[(length(fn)-config$file.limit+1):length(fn)]
  }
  
  # Remove address and suffix, get pure sample names
  samples <- sapply(fn, function(x) {
  ...
  if (length(fn) == 0) {
    return(fn)  
  } else if (any(table(samples) > 1)) {
    multiple <- which(table(samples) > 1)
    
    say('Error!')
    say('Multiple files found for samples:', type = 'error')
...
# Concatenate logs
concatLogs <- function(output.dir, logfile, delete.original = TRUE) {
  ...
  if(delete.original) unlink(logfile, force = TRUE)
}
