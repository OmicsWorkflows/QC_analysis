...
qcCorr <- function(s, ref.matrix, format) {
  ...
  for (j in 1:nrow(comp)) {
    ...
    matches <- mzi > tmp$mz - opts$qc.mz.tolerance & mzi < tmp$mz + opts$qc.mz.tolerance
...
processIdMzML <- function(x, i, ref, identify.bool) {
  ...
  xc <- chromatogram(x, mz = c(m-opts$qc.mz.tolerance, m+opts$qc.mz.tolerance))
  xc[,1]@intensity[is.na(xc[,1]@intensity)] <- 0
  xc[,1]@rtime <- xc[,1]@rtime/60
  
  if(identify.bool) {  
    # Filter out only data for relevant m/z
    px <- x |> filterPrecursorMz(m, ppm = (opts$qc.mz.tolerance/m)*1e6)
    mxx <- mx |>
      dplyr::filter(id == i) |>
      select(mz, intensity)
...
calculateQCmzML <- function(x, dir, output.dir, matrix, identify.bool = config$qc.identify, 
                            sample.name = 'sample') {
  ...
  tryCatch({
    # read .mzML files
    readMS <- readMSData(x, mode = "onDisk")
    
    # return success message
    say(paste0('.mzML files for QC peptide XIC chromatogram computation read from ', dir, ' for sample:'))
    say(names(x), type = 'input')
    
    say('QC peptide data calculated and saved as:')
    
    tmp <- readMS |> filterFile(paste0(names(x), '.mzML'))
    
    d <- processQCfileMzML(tmp, ref, identify.bool)
    saveDataCsv(names(x), d, output.dir)
    d <- mutate(d, !!sample.name := names(x))

    return(d)
  }, error = function(e){
    say('Warning!')
    say(paste('Could not process QC data for sample', names(x)), type = 'warn')
    return(NULL)
  })
}
...
processIdRaw <- function(x, i, ref, identify.bool) {
  ...
  xc <- readChromatogram(x, mass = m, tol = (opts$qc.mz.tolerance/m)*1e6, type = 'xic')
  ...
  if(identify.bool){
    ...
    scannum <- scan.df |>
      dplyr::filter(precursorMass >= m-opts$qc.mz.tolerance & precursorMass <= m+opts$qc.mz.tolerance) |>
      pull(scan)
...
calculateQCraw <- function(x, dir, matrix, identify.bool,
  ...
  tryCatch({
    l <- lapply(ref$ids, function(id){
      processIdRaw(x, id, ref, identify.bool)
    })
  ...
  if(length(flist$calc) > 0) {
    if(opts$file.format == '.mzML') {
      res <- lapply(setNames(names(flist$calc), names(flist$calc)), 
                    function(y) { calculateQCmzML(flist$calc[y], dir = opts$file.input,
                              identify.bool = opts$qc.identify, output.dir = opts$qc.dir,
                              matrix = opts$qc.reference.matrix) })
    } else if(opts$file.format == '.raw'){
      res <- lapply(setNames(names(flist$calc), names(flist$calc)), 
                    function(y) { calculateQCraw(flist$calc[y], dir = opts$file.input,
                       matrix = opts$qc.reference.matrix, identify.bool = opts$qc.identify,
...
createSeparateQCplot <- function(x, df, title, i, mark.peak, config, by, batch, inset.bool = TRUE) {
  ...
  tmp$sample <- sapply(tmp$sample, function(x) opts$sample.table$sample.adj[opts$sample.table$sample == x])
  
  # get QC color
  col.i <- i %% length(qcColors)
  if(col.i == 0) col.i <- length(qcColors)
  ...
  if(by == 'QC'){
    col <- rep(qcColors[col.i], times = length(unique(tmp[,group.col])))
    
    p <- p + 
      facet_wrap(sample~., scales = 'free_y', ncol = config$plot.max.columns) +
      theme(legend.position = 'none') +
      labs(subtitle = opts$plot.subtitle,
           title = title) +
  ...
  p.info <- list(plot = list(p),
                 sample = x,
                 height = (120 * ceiling(length(unique(tmp[,group.col]))/config$plot.max.columns)) + 100,
                 stats = inset.stats)
...
plotQc <- function(df, opts, by) {
  ...
  # cannot perform identification for .d and .csv files
  if(opts$qc.identify == TRUE & opts$file.format %in% c('.d', '.csv')) {
    say('Warning!')
    say('Currently not possible to identify QC peptides, proceeding without identification', type = 'warn')
    opts$qc.identify <- FALSE
  }
  ...
  for (x in unique(df[,by])) {
    res <- createSeparateQCplot(x, df, paste('QC peptide', x), which(unique(df[,by]) == x), opts$qc.identify, opts, by, opts$file.batch, opts$qc.identify)
    plot_list[[length(plot_list)+1]] <- tibble(name = x,
                                               link = paste0('qc_peptides_', x),
                                               plot = list(res$plot[[1]]),
                                               filename = paste0(removeSpecialCharacters(opts$file.batch), '_QC_', x ,'.png'),
                                               width = NA,
                                               height = res$height,
                                               type = 'QC peptides', 
                                               level = 'MS',
  ...
  # Modify plot_stats variable if necessary
